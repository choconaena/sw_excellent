...
def sttOriginToAbstract(stt_id, original_stt_text):
    stt_id = 1
    ...
    # 2. LLM 프롬프트 구성
    summarization_prompt = f"""반드시 한글로 작성해. 너는 전문 사회복지사야. 아래 [상담 내용]을 읽고, subject, abstract, detail로 구성된 구조화된 요약을 작성해줘. 
    만약 subject가 여러개라면 이 JSON형식은 여러번 반복되어도 괜찮아. 응답을 줄때는 아래 형식에 맞게 써 반드시. 
    응답을 줄때는 다른 내용은 절대 포함하지말고 [답변예시]를 참고해.
    ...
    # 4. 요약 실행
    summary_result = call_ollama("llama2:7b", summarization_prompt)
