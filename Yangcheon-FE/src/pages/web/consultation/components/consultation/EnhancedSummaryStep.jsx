...
import { getSttAbstract } from "../../../../../api/consultation";
  ...
  const fetchSummary = useCallback(
    async (retryCount = 0) => {
      if (!reportId) {
        ...
        setPendingMsg("");
        setError(
          "요약을 조회할 reportId가 없습니다. (STT 세션을 먼저 시작해 주세요)"
        );
        return;
      }
      ...
      try {
        ...
        if (res?.status && Array.isArray(res.items) && res.items.length > 0) {
          ...
          setPendingMsg(msg);
          send({
            msg: { msg_type: "data", content: { summary: { pending: msg } } },
          });
        ...
        setError(em);
        send({
          msg: { msg_type: "data", content: { summary: { error: em } } },
        });
      } finally {
        setLoading(false);
      }
